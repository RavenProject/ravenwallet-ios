...
int PaymentProtocolTests() {
    ...
    if (len != sizeof(buf3) || memcmp(buf3, buf4, len) != 0) // check if parse/serialize produces same result
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolRequestParse/Serialize() test 1\n", __func__);
    ...
    if (i != 3) r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolRequestCert() test 1\n", __func__);
    
    if (req->details->expires == 0 || req->details->expires >= time(NULL)) // check that request is expired
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolRequest->details->expires test 1\n", __func__);
    ...
    if (len != sizeof(buf5) - 1 || memcmp(buf5, buf6, len) != 0) // check if parse/serialize produces same result
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolACKParse/Serialize() test\n", __func__);
    
    printf("\n");
    if (ack->memo) printf("%s\n", ack->memo);
    // check that memo is not NULL
    if (! ack->memo) r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolACK->memo test\n", __func__);
    ...
    if (len != sizeof(buf7) - 1 || memcmp(buf7, buf8, len) != 0) // check if parse/serialize produces same result
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolRequestParse/Serialize() test 2\n", __func__);
    i = 0;
    ...
    if (i != 2) r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolRequestCert() test 2\n", __func__);
    
    if (req->details->expires == 0 || req->details->expires >= time(NULL)) // check that request is expired
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolRequest->details->expires test 2\n", __func__);
    ...
    if (len > 0)
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolRequestParse/Serialize() test 3\n", __func__);
    
    printf("                                    ");
    return r;
}

int PaymentProtocolEncryptionTests() {
    int r = 1;
    ...
    if (! req) r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolInvoiceRequestNew() test\n", __func__);
    ...
    if (! req || memcmp(req->senderPubKey.pubKey, senderKey.pubKey, 33) != 0)
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolInvoiceRequestSerialize/Parse() test\n", __func__);
    ...
    BRPaymentProtocolMessage *msg1 = BRPaymentProtocolMessageNew(PaymentProtocolMessageTypeACK,
                                                                 (uint8_t *) buf, sizeof(buf) - 1, 1, NULL,
                                                                 id, sizeof(id));
    
    if (! msg1) r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolMessageNew() test\n", __func__);
    ...
    if (! msg1 || msg1->msgLen != sizeof(buf) - 1 || memcmp(buf, msg1->message, sizeof(buf) - 1) != 0)
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolMessageSerialize/Parse() test\n", __func__);
    
    if (msg1) BRPaymentProtocolMessageFree(msg1);
    
    BRPaymentProtocolEncryptedMessage *msg2 =
            BRPaymentProtocolEncryptedMessageNew(PaymentProtocolMessageTypeACK, (uint8_t *) buf, sizeof(buf) - 1,
                                                 &receiverKey, &senderKey, time(NULL), id, sizeof(id), 1, NULL);

    if (! msg2) r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolEncryptedMessageNew() test\n", __func__);
    ...
    if (! msg2 || msg2->msgLen != sizeof(buf) - 1 + 16)
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolEncryptedMessageSerialize/Parse() test\n", __func__);

    if (msg2 && !BRPaymentProtocolEncryptedMessageVerify(msg2, &receiverKey))
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolEncryptedMessageVerify() test\n", __func__);
    ...
    if (outLen != sizeof(buf) - 1 || memcmp(buf, out, outLen) != 0)
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolEncryptedMessageDecrypt() test 1\n", __func__);
    
    if (msg2) outLen = BRPaymentProtocolEncryptedMessageDecrypt(msg2, out, sizeof(out), &senderKey);
    
    if (outLen != sizeof(buf) - 1 || memcmp(buf, out, outLen) != 0)
        r = 0, fprintf(stderr, "***FAILED*** %s: PaymentProtocolEncryptedMessageDecrypt() test 2\n", __func__);
...
int RunTests() {
    ...
    printf("PaymentProtocolTests...           ");
    printf("%s\n", (PaymentProtocolTests()) ? "success" : (fail++, "***FAIL***"));
    printf("PaymentProtocolEncryptionTests... ");
    printf("%s\n", (PaymentProtocolEncryptionTests()) ? "success" : (fail++, "***FAIL***"));
    printf("\n");

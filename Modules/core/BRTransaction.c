...
uint32_t BRRand(uint32_t upperBound) {
    ...
    do { // to avoid modulo bias, find a rand value not less than 0x100000000 % upperBound
        r = rand();
             ...
             ((0xffffffff - upperBound * 2) + 1) % upperBound); // (((0xffffffff - x*2) + 1) % x) == (0x100000000 % x)
...
static size_t _TxInputData(const BRTxInput *input, uint8_t *data, size_t dataLen) {
    size_t off = 0;
...
static size_t
_TransactionWitnessData(const BRTransaction *tx, uint8_t *data, size_t dataLen, size_t index, int hashType) {
    BRTxInput input;
    ...
    if (!anyoneCanPay) {
        ...
        if (data && off + sizeof(UInt256) <= dataLen) SHA256_2(&data[off], buf, sizeof(buf)); // inputs hash
    ...
    if (!anyoneCanPay && sigHash != SIGHASH_SINGLE && sigHash != SIGHASH_NONE) {
        ...
        if (data && off + sizeof(UInt256) <= dataLen) SHA256_2(&data[off], buf, sizeof(buf)); // sequence hash
    ...
    off += _TxInputData(&input, (data ? &data[off] : NULL), (off <= dataLen ? dataLen - off : 0));

    if (sigHash != SIGHASH_SINGLE && sigHash != SIGHASH_NONE) {
        size_t bufLen = _TransactionOutputData(tx, NULL, 0, SIZE_MAX);
        uint8_t _buf[(bufLen <= 0x1000) ? bufLen : 0], *buf = (bufLen <= 0x1000) ? _buf : malloc(bufLen);

        bufLen = _TransactionOutputData(tx, buf, bufLen, SIZE_MAX);
        if (data && off + sizeof(UInt256) <= dataLen) SHA256_2(&data[off], buf, bufLen); // SIGHASH_ALL outputs hash
        if (buf != _buf) free(buf);
    } else if (sigHash == SIGHASH_SINGLE && index < tx->outCount) {
        uint8_t buf[_TransactionOutputData(tx, NULL, 0, index)];
        size_t bufLen = _TransactionOutputData(tx, buf, sizeof(buf), index);

        if (data && off + sizeof(UInt256) <= dataLen) SHA256_2(&data[off], buf, bufLen); //SIGHASH_SINGLE outputs hash
...
static size_t _TransactionData(const BRTransaction *tx, uint8_t *data, size_t dataLen, size_t index, int hashType) {
    BRTxInput input;
    int anyoneCanPay = (hashType & SIGHASH_ANYONECANPAY), sigHash = (hashType & 0x1f);
    size_t i, off = 0;

    if (hashType & SIGHASH_FORKID) return _TransactionWitnessData(tx, data, dataLen, index, hashType);
    ...
    if (sigHash != SIGHASH_SINGLE && sigHash != SIGHASH_NONE) { // SIGHASH_ALL outputs
        ...
        off += _TransactionOutputData(tx, (data ? &data[off] : NULL), (off <= dataLen ? dataLen - off : 0), SIZE_MAX);
        ...
        off += _TransactionOutputData(tx, (data ? &data[off] : NULL), (off <= dataLen ? dataLen - off : 0), index);
    } else
        off += BRVarIntSet((data ? &data[off] : NULL), (off <= dataLen ? dataLen - off : 0), 0); //SIGHASH_NONE outputs
...
BRTransaction *BRTransactionParse(const uint8_t *buf, size_t bufLen) {
    ...
    for (i = 0; off <= bufLen && i < tx->outCount; i++) {
        ...
        // Get assets output count if assets are activated and if there is any!
    ...
    // Parse assets in tx
    if (tx->blockHeight >= ASSET_ACTIVATION &&
        tx->asstCount > 0) {// if assets are deployed and tx has assets outputs (asstcount > 0)
        array_new(tx->asset, tx->asstCount);
        for (i = 0, j = 0; i < tx->outCount && j < tx->asstCount; i++) {
            // if (!output->script || output->scriptLen == 0) return tx; // WTF is this return tx, there is still things to be done.
            if (!output->script || output->scriptLen == 0) continue; // This may be redundant
            output = &tx->outputs[i];
            if (IsScriptAsset(output->script, output->scriptLen)) {
                GetAssetData(output->script, output->scriptLen, &tx->asset[j]);
                j++;
            }
        }
    }
...
size_t BRTransactionSerialize(const BRTransaction *tx, uint8_t *buf, size_t bufLen) {
    assert(tx != NULL);
    return (tx) ? _TransactionData(tx, buf, bufLen, SIZE_MAX, SIGHASH_ALL) : 0;
}
...
int BRTransactionSign(BRTransaction *tx, BRKey *keys, size_t keysCount) {
    ...
    for (i = 0; tx && i < keysCount; i++) {
        if (!BRKeyAddress(&keys[i], addrs[i].s, sizeof(addrs[i]))) addrs[i] = ADDRESS_NONE;
    }
...
#warning TODO: OP_EQUALVERIFY condition doesn't trigger for assets input
    ...
    if (tx && BRTransactionIsSigned(tx)) {
        uint8_t data[_TransactionData(tx, NULL, 0, SIZE_MAX, 0)];
        size_t len = _TransactionData(tx, data, sizeof(data), SIZE_MAX, 0);

...
let detailAssetHeaderHeight: CGFloat = 152.0
let detailAssetFooterHeight: CGFloat = 67.0

class DetailAssetViewController : UIViewController, Subscriber {

    //MARK: - Public
    init(asset: Asset) {
        self.asset = asset
        self.headerView = AssetHeaderView(asset: asset)
        self.footerView = AssetFooterView(asset: asset)
        super.init(nibName: nil, bundle: nil)
        self.assetTransactionsTableView = AssetTransactionsTableVC(asset: asset, didSelectTransaction: didSelectTransaction)
    ...
    private let headerView: AssetHeaderView
    private let footerView: AssetFooterView
    private let transitionDelegate = ModalTransitionDelegate(type: .transactionDetail)
    private var assetTransactionsTableView: AssetTransactionsTableVC!
    ...
    private func addTransactionsView() {
        ...
        addChild(assetTransactionsTableView, layout: {
            if #available(iOS 11.0, *) {
                assetTransactionsTableView.view.constrain([
                    assetTransactionsTableView.view.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor),
                assetTransactionsTableView.view.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor),
                assetTransactionsTableView.view.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor),
                assetTransactionsTableView.view.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor),
                ])
            } else {
                assetTransactionsTableView.view.constrain(toSuperviewEdges: nil)
            }
        })
    }
    ...
    private func showJailbreakWarnings(isJailbroken: Bool) {
        ...
        let totalSent = 0//BMEX walletManager.wallet?.totalSent ?? 0
    ...
    private func showSearchHeaderView() {
        ...
        var contentInset = self.assetTransactionsTableView.tableView.contentInset
        var contentOffset = self.assetTransactionsTableView.tableView.contentOffset
        contentInset.top += navBarHeight
        contentOffset.y -= navBarHeight
        self.assetTransactionsTableView.tableView.contentInset = contentInset
        self.assetTransactionsTableView.tableView.contentOffset = contentOffset

...
class SyncingHeaderView: UIView, Subscriber {
    ...
    private let date = UILabel(font: .customBody(size: 12.0), color: UIColor.fromHex("828282"))
    ...
    private func setInitialState() {
        ...
        Store.subscribe(self, selector: { $0[self.currency].syncState != $1[self.currency].syncState },
                        callback: { state in
                            self.syncState = state[self.currency].syncState
        })
        
        Store.subscribe(self, selector: {
            return $0[self.currency].lastBlockTimestamp != $1[self.currency].lastBlockTimestamp
        }, callback: {
            self.lastBlockTimestamp = $0[self.currency].lastBlockTimestamp
        })
    }

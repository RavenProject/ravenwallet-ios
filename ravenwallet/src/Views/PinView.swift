...
class PinView : UIView {
    ...
    init(style: PinViewStyle, length: Int) {
        ...
        switch style {
        case .create:
            unFilled = (0...(length-1)).map { _ in Rect(color: .borderGray) }
        case .login:
            unFilled = (0...(length-1)).map { _ in Rect(color: .white) }
        }
        filled = (0...(length-1)).map { _ in Rect(color: .orange) }
    ...
    //MARK: - Private
    private let unFilled: [Rect]
    private var filled: [Rect]
    ...
    private func addCircleContraints(_ circles: [Rect]) {

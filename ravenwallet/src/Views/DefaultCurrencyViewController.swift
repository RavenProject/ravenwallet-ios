...
class DefaultCurrencyViewController : UITableViewController, Subscriber {
    ...
    init(walletManager: WalletManager) {
        ...
        self.rates = Currencies.rvn.state.rates.filter { $0.code != C.rvnCurrencyCode }
    ...
    override func viewDidLoad() {
        ...
        Store.subscribe(self, selector: { $0[Currencies.rvn].maxDigits != $1[Currencies.rvn].maxDigits }, callback: { _ in
    ...
    private func setExchangeRateLabel() {
        if let currentRate = rates.filter({ $0.code == defaultCurrencyCode }).first {
            let amount = Amount(amount: C.satoshis, rate: currentRate, maxDigits: Currencies.rvn.state.maxDigits, currency: Currencies.rvn)
            let bitsAmount = Amount(amount: C.satoshis, rate: currentRate, maxDigits: Currencies.rvn.state.maxDigits, currency: Currencies.rvn)
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        ravencoinSwitch.valueChanged = strongify(self) { myself in
            ...
            if newIndex == 1 {
                ...
                Store.perform(action: WalletChange(Currencies.rvn).setMaxDigits(2))
            }
        }
